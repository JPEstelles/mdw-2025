...
const register = async (req, res) => {
    try {
        ...
        const existingUser = await userModel_1.User.findOne({ $or: [{ username }, { email }] });
        if (existingUser) {
        ...
        // Generar tokens
        const accessToken = jsonwebtoken_1.default.sign(tokenPayload, ACCESS_TOKEN_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRES });
        const refreshToken = jsonwebtoken_1.default.sign({ _id: user._id.toString() }, // Solo _id para refresh token está bien
...
const login = async (req, res) => {
    try {
        ...
        // Generar tokens
        const accessToken = jsonwebtoken_1.default.sign(tokenPayload, ACCESS_TOKEN_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRES });
        const refreshToken = jsonwebtoken_1.default.sign({ _id: user._id.toString() }, REFRESH_TOKEN_SECRET, { expiresIn: REFRESH_TOKEN_EXPIRES });
        // Guardar tokens en cookies
        setTokenCookies(res, accessToken, refreshToken);
